...
export default function SkillsPage() {
  ...
  useEffect(() => {
    ...
    const skillNames = JSON.parse(extractedSkills)
    setSkills(skillNames.map((name: string) => ({ name, level: "beginner" as const })))
  }, [router])

  const updateSkillLevel = (skillName: string, level: "beginner" | "intermediate" | "advanced") => {
    setSkills(skills.map((skill) => (skill.name === skillName ? { ...skill, level } : skill)))
  }
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4">
              {skills.map((skill, index) => (
                <div
                  key={skill.name}
                  className={`flex items-center justify-between p-4 border rounded-lg bg-white hover:bg-gray-50 transition-all duration-300 hover-lift animate-slideInLeft animate-delay-${Math.min(index * 100, 500)}`}
                >
                  ...
                  <Select value={skill.level} onValueChange={(level: any) => updateSkillLevel(skill.name, level)}>
